...
def position_mapping(species,chipName,dbType,region,log=0):
    ...
    **Map positions to gene structures (exon, intron, upstream or downstream).**
    ...
    t1=time.time()        
    if dbType=='ensembl':        
        log1=open(os.environ['PYDATA']+"/"+species+'/log/'+'%s_ensembl_position_mapping_r%s_log.txt'%(chipName,region),'a')
        
        path=os.environ['PYDATA']+"/"+species+'/'+dbType+'/'+species+'_genes_by_region.bkdb'                
        genesByRegionDB=bsddb.btopen(path,'r')        
        ...
        exonsByGeneDB=bsddb.btopen(path)
        ...
        log1=open(os.environ['PYDATA']+"/"+species+'/log/'+'%s_aceview_position_mapping_log.txt'%chipName,'a')
                ...
                if geneIndexes!=[]:                        
                    ...
                    if useCurrentProbe==1:                        
                        ...
                        for geneIndex in range(len(geneIDs)):
                            ...
                            try:                 
                                exons=cPickle.loads(exonsByGeneDB[geneID])
                                processExons=1
                                if exons.IDs==[]:                                   
                                    for i in range(len(exons.indexes)):
                                        exons.IDs.insert(i,geneID+'.exon%u'%exons.indexes[i])
                                        exonsByGeneDB[geneID]=cPickle.dumps(exons,protocol=-1)                                        
                            except:
                                processExons=0
...
#                    print 'updown: genes %s localisation %s'%(updownGeneIDs,updownGeneLocalisations)
    ...
    if log!=0:
        if dbType=='ensembl':
            log.write('%s\t%s\t\t21\t%s_exons_by_gene.bkdb\t\tensembl update r %s\t%.2f\n'%(date.today(),species,species,region,t4-t1))
            log.write('%s\t%s\t%s\t21\t%s_probe.bkdb\t%u probe pb, %u exon pb\tensembl update r %s\t%.2f\n'%(date.today(),species,chipName,chipName,failedExonNb,failedProbeNb,region,t4-t1))
        else:
            log.write('%s\t%s\t\t24\t%s_exons_by_gene.bkdb\t\taceview update r %s\t%.2f\n'%(date.today(),species,species,region,t4-t1))
            log.write('%s\t%s\t%s\t24\t%s_probe.bkdb\t%u probe pb, %u exon pb\taceview update r %s\t%.2f\n'%(date.today(),species,chipName,chipName,failedExonNb,failedProbeNb,region,t4-t1))                        
...
def position_gopmapping(species,chipName,region,log=0):
    ...
    **Map positions to group of probes (GOPs).**    
    ...
    gop_log=open(os.environ['PYDATA']+"/"+species+'/log/'+species+'_ensembl_position_gopmapping_log.txt','a')
...
#                    sys.exit()                                                                                                                                                                                                        
        ...
        if log!=0:
            log.write('%s\t%s\t%s\t22\t%s_probe.bkdb\t%u probe pb,\tensembl update r %s\t%.2f\n'%(date.today(),species,chipName,chipName,failedProbeNb,region,t2-t1))                        
...
def assign_probeset(species,chipName,dbType,psRange=0,aceVersion='',log=0):
    """
    **Assign probe set to gene(s) or GOP(s) according to the number of targeting probes.**
    ...
    for psRank in psRange:    
        ...
        try:        
            ...
            if log!=0:
                if dbType=='ensembl':
                    log.write('%s\t%s\t\t23\t%s_probeset.bkdb\t\tensembl update\t"CPICKLE ERROR FOR PROBESET %s (rank %u)\n'%(date.today(),species,species,probesetID,psRank))
                elif dbType=='aceview':
                    log.write('%s\t%s\t\t25\t%s_probeset.bkdb\t\tensembl update\t"CPICKLE ERROR FOR PROBESET %s (rank %u)\n'%(date.today(),species,species,probesetID,psRank))                                 
            sys.exit()                
        if dbType=='ensembl':
    ...
    #                    if genes[geneIndex].firstStructureIndexes[probeIndex]==[]:                        
                ...
                if probe.genes4splice !={} and probe.genes4splice is not None:
                    # targetGenes have normally been already been created                                               
                    for geneID in probe.genes4splice.keys():                            
                        try:                            
                            ...
                            logFile.write('genes4splice filled in a previous PsRank: PsRank %u ProbeSet %s GeneID %s Probe %s\n'%(psRank,probesetID,geneID,probeIDs[probeIndex]))        
...
def probesets_by_gene(species,chipName,aceFlag,log=0):
    """
...
#    position_mapping('mouse','m8','ensembl','9')        
#    position_gopmapping('mouse','m8','1')
#    erase_gop('mouse','m8')
#    position_gopmapping('mouse','m8','1')
#    assign_probeset('human','m2','ensembl')
    pass
...
#    write_probeset_list('mouse','m8')
    ...
    assign_probeset('mouse','m8','ensembl',psRange=[4064])
